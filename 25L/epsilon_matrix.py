...
class EpsilonMatrix(object):
    def __init__(self, targets, nonpos=False):
        ...
        for i in range(self.N):
            for j in range(self.N):
                if self.index[i, j] >= 0:
                    V2Mlil[self.N * i + j, self.index[i, j]] = 1.0
        ...
        self.Vt = [
            np.array(
                [
                    1.0
                    if self.targets[a, i] > 0.0 and self.targets[a, j] > 0.0
                    else 0.0
                    for i in range(self.N)
                    for j in range(i, self.N)
                    if self.index[i, j] >= 0
    ...
    def cp_variance(self):
        return (
            cp.sum_squares(cp.multiply(self.VWh, self.vec - self.cp_mean()))
            / self.N**2
        )
    ...
    def cp_target_diag(self):
        return [
            cp.hstack(
                [
                    self.vec[self.index[i, i]]
                    for i in range(self.N)
                    if self.targets[a, i] > 0.0
                ]
            )
            for a in range(self.n)
        ]
    ...
    def cp_target_msds(self, mus):
        return [
            cp.sum_squares(cp.multiply(self.VWah[i], self.vec - mus[i]))
            / self.VWa[i].sum()
            for i in range(self.n)
        ]
    ...
    def set_close_elements_equal(self, eps_ref, tol=1.0e-3):
...
class MuVector(object):
    def __init__(self, targets):
        ...
        for i in range(self.N):
            self.V2M[i * self.N : (i + 1) * self.N, i] = 1.0
...
def gen_symm_gaussian_noise(eps, sigma):
    v = np.array([random.gauss(0.0, sigma) for i in range(eps.neps)])
    return get_symm_matrix(v, eps.index)


def count_close_elements(eps, eps_ref, tol=1.0e-3):
    v_ref = get_symm_vector(eps_ref, eps.index)
    return sum(
        1 for i in range(eps.neps) if math.fabs(eps.vec.value[i] - v_ref[i]) <= tol
    )
